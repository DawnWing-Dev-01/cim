...
public class CaptchaBuilder {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(CaptchaBuilder.class);
	...
	public CaptchaBuilder(int width, int height, int codeCount) {
		this.width = width;
		this.height = height;
	...
	public void write(OutputStream sos) {
		try {
			...
			logger.error("verified code write error...", e);
		} finally {
		    IoUtils.unifyClose(sos);
		}
	}
	...
	public void getCode3(HttpServletRequest req, HttpServletResponse response,
		...
		CaptchaBuilder cb = new CaptchaBuilder(110, 35, 5, 10);
		session.setAttribute("code", cb.getCode());
		cb.write(response.getOutputStream());
	}
}
